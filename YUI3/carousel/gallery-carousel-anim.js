...
Y.CarouselAnimPlugin = Y.extend(CarouselAnimPlugin, Y.Plugin.Base, {
    ...
    initializer: function (config) {
        ...
        var autoPlayInterval, carousel = this.get("host");
        autoPlayInterval = carousel.get("autoPlayInterval");
        if (autoPlayInterval > 0) {
            carousel.set("autoPlayTimer", Y.later(autoPlayInterval, carousel, carousel.scrollForward));
        }
    },
    ...
    _afterAnimEnd: function (pos) {
        var autoPlayInterval, first, self = this, carousel = self.get("host");
        autoPlayInterval = carousel.get("autoPlayInterval");
        carousel.set("selectedItem", pos);
        first = carousel.getFirstVisible();
        self.fire(AFTERSCROLL_EVENT,
                { first: first,
                  last: first + carousel.get("numVisible") });
        if (carousel.get("autoPlayTimer")) {
            carousel.get("autoPlayTimer").cancel();
        }
        if (autoPlayInterval > 0) {
            carousel.set("autoPlayTimer", Y.later(autoPlayInterval, carousel, carousel.scrollForward));
        }
