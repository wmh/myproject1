...
YUI().use('node', 'base', 'plugin', function(Y) {
    /*
     * Advanced Plugins - 添加行為 (跟上面一個類似)
     */
    function MakeFloating(config) {
        this.config = config;
    }
    MakeFloating.NAME = 'makeFloatingPlugin';
    MakeFloating.NS = 'makeFloating';
    ...
    // Extend Plugin.Base
    MakeFloating.prototype = {};
    MakeFloating.prototype.top = function () {
        this.config.host.setStyle("backgroundColor", "red");
    };

    var myNode = Y.one("#t1");
    myNode.plug(MakeFloating);
    myNode.makeFloating.top();
